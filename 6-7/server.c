...
const char *observers_shared_object = "/posix-observers-shared";

int pipe_fd[2];

void writeEventToPipe(struct Event *event) {
    if (write(pipe_fd[1], event, sizeof(*event)) == -1) {
...
int createServerSocket(in_addr_t sin_addr, int port) {
    ...
    if ((server_socket = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1) {
        perror("Creating of server socket is unabled");
        exit(-1);
    }
    ...
    if (bind(server_socket, (struct sockaddr *)&server_address, sizeof(server_address)) == -1) {
        perror("binding address is unabled");
        exit(-1);
    }

    if (listen(server_socket, MAXPENDING) == -1) {
        perror("listening to server socket is unabled");
        exit(-1);
    }
...
int acceptClientConnection(int server_socket) {
    ...
    if ((client_socket =
             accept(server_socket, (struct sockaddr *)&client_address, &address_length)) == -1) {
        perror("accepting of client connection is unabled");
        exit(-1);
    }
...
void handle(int client_socket, sem_t *semaphores, int *field, struct FieldSize field_size) {
    ...
    if ((bytes_received = recv(client_socket, buffer, sizeof(struct Task), 0)) == -1) {
        perror("bad recv()");
        exit(-1);
    }
...
struct Observer *getObserversMemory() {
    ...
    if ((shmid = shm_open(observers_shared_object, O_CREAT | O_RDWR, 0666)) == -1) {
        perror("Can't connect to shared memory");
        exit(-1);
    } else {
        if (ftruncate(shmid, 100 * sizeof(struct Observer)) == -1) {
            perror("Can't resize shared memory");
            exit(-1);
        }
        ...
        printf("Open memory with observers\n");
    }
...
sem_t *createSemaphoresSharedMemory(int sem_count) {
    ...
    if ((sem_main_shmid = shm_open(sem_shared_object, O_CREAT | O_RDWR, 0666)) == -1) {
        ...
        printf("Open memory for semaphores\n");
    }
...
void waitChildProcessess() {
    while (children_counter > 0) {
        ...
        if (child_id < 0) {
            perror("Waiting to child proccess is unabled");
            exit(-1);
        } else {
            children_counter--;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        fprintf(stderr,
                "4 args: Server address and port, Observer Port and Square side size\n",
                argv[0]);
        exit(1);
    }
    ...
    while (1) {
        ...
        pid_t child_id;
        if ((child_id = fork()) < 0) {
            perror("Creating of child for handling new connection is unabled");
            exit(-1);
        } else if (child_id == 0) {
